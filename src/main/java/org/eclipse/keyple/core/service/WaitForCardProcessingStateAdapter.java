 ...
 *       WAIT_FOR_CARD_DETECTION according to the {@link ObservableCardReader.PollingMode} setting.
...
final class WaitForCardProcessingStateAdapter extends AbstractObservableStateAdapter {
  ...
  void onEvent(ObservableLocalReaderAdapter.InternalEvent event) {
    ...
    switch (event) {
      case CARD_PROCESSED:
        if (this.getReader().getPollingMode() == ObservableCardReader.PollingMode.REPEATING) {
