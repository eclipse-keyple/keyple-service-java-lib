...
## [Unreleased]
### Fixed
- Corrects an anomaly in communication between the server and a client using a version of the Distributed JSON API prior
  to 2.0, when the card selection request sent by the server does not contain an additional card request.
